...
try:
    ...
    print(colored('[!] warning: cannot import "librosa", use my own "resampling" implementation', 'red'))
...
def _resample(y, orig_sr, target_sr, fix=True, scale=False):
    ...
    if fix:
        y_hat = _fix_length(y_hat, n_samples)
    if scale:
        y_hat /= np.sqrt(ratio)
...
def audioread(path, fs=16000):
    ...
    if sr != fs:
        ...
        if librosa_imported:
            ...
            wave_data = _resample(wave_data, sr, fs)
...
def get_one_spk_noise(clean, noise, snr, scale):
    ...
    return noisy, clean, gen_noise, noisy_scale

def generate_data(clean_path, start_time, noise_path, rir_path, snr, scale, segment_length=16000*4, channels=8):
    clean = get_firstchannel_read(clean_path)
    # chunk the clean wav
    clean = clip_data(clean, start_time, segment_length)
    ...
    inputs, labels, noises, noisy_scale = get_one_spk_noise(clean, noise, snr, scale)
    return inputs, labels, noises, clean_early*noisy_scale
...
def mix_func(line, save_dir, chunk, sample_rate):
    try:
        ...
        # L x C
        inputs, labels, noises, noreverb_ref = generate_data(clean_path, int(start_time), noise_path, rir_path, 
...
def wrap_mix_data(clean_path, start_time, noise_path, rir_path, snr, scale, chunk=6, sample_rate=16000):
    segment_length = int(chunk * sample_rate)
    # L x C
    inputs, labels, noises, noreverb_ref = generate_data(clean_path=clean_path, 
    ...
    return inputs.astype(np.float32), labels[:,0].astype(np.float32), noises[:,0].astype(np.float32)
